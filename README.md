...
It is a good safety measure to run Inference engines with tooling enabled inside a container because 
...
*   **`3-agilengine/Agentic-local-MCP.py`**
...
*   **`3-agilengine/Atlassian-ingress-MCP.py`**
    ...
    **Internal Workings:** It authenticates with Atlassian APIs using HTTP Basic Auth. It exposes Jira active sprints as a read-only resource by fetching incomplete issues from the specified Jira project. It provides agentic tools to sync local tasks from `project_state.json` to automatically create Jira tickets, and tools to publish the `DAILY_SYNTHESIS.md` generated by `Agentic-Agile.py` directly to a Confluence page.

*   **`3-agilengine/Automatic-Unittests.py`**
    **Functional Description:** A parallelized LLM test generation and execution pipeline that parses extracted code blocks from a markdown document, requests unit tests from multiple local inference workers concurrently, and executes the generated test suites to validate functionality.
    **Internal Workings:** It parses a markdown source file to extract code artifacts, dispatching test generation tasks to local LLM endpoints using a ThreadPoolExecutor with exponential backoff. It then sanitizes any requirement files before executing the tests in subprocesses, aggregating the outputs into JSON and CSV summary reports.
...
4. **Integrations & Interfaces:** Start bridge services like `3-agilengine/Agentic-local-MCP.py` and `3-agilengine/Atlassian-ingress-MCP.py` to expose local state to external tools, and run `9-misc/local-discord-bot.py` to provide a conversational interface.
